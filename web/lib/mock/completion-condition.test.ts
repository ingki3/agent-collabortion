...
async function create(cc: unknown, extra: Record<string, unknown> = {}) {
  ...
  return call("POST", `/__mock/workspaces/${await ws()}/seed-room`, {
...
function listen(workspaceId: string) {
  const frames: { type: string; room_id: string | null; payload: Record<string, unknown> }[] = [];
...
describe("createSession — 리뷰어 검사(422 두 코드)", () => {
  ...
  it("리뷰어가 참여자가 아니면 422 reviewer_not_participant", async () => {
    ...
    const wsId = await ws();
    const res = await call("POST", `/__mock/workspaces/${wsId}/seed-room`, {
...
describe("옛 updateSession 목 길(PATCH /__mock/rooms/{id}/legacy — updateWork 가 시드한 방의 미션에 쓴다) — completion_condition 은 active·paused 에서도(Director), 진행률 재계산 + SSE", () => {
  ...
  it("active 에서 리뷰어를 넣어 고치면 200 · 진행률 재계산 · work.completion_progress 한 번(옛 session.completion_progress 는 R4 에서 지워졌다)", async () => {
    const s = await activeSession();
    const [lead] = await agents();
    const frames = listen(s.workspace_id);
    const res = await call("PATCH", `/__mock/rooms/${s.id}/legacy`, { body: { completion_condition: { op: "and", conditions: [{ type: "artifact_submitted", who: "assignee" }, { type: "agent_approval", agent_id: lead.id }, { type: "user_approval" }] } } });
    ...
    const ev = frames.filter((f) => f.type === "work.completion_progress");
    expect(ev).toHaveLength(1);
    expect(ev[0].room_id).toBe(s.id);
    expect(Object.keys(ev[0].payload).sort()).toEqual(["completion_progress", "room_id", "work_id"]);
    expect(frames.some((f) => f.type.startsWith("session."))).toBe(false);
  ...
  it("이미 충족된 원자는 그대로 유지된다(met·met_at·met_by)", async () => {
    const s = await activeSession();
    const [lead] = await agents();
    await must("POST", `/__mock/rooms/${s.id}/seed-legacy-condition`, { body: { met_artifact: true } });
    const before = await must<Session>("GET", `/__mock/rooms/${s.id}/legacy`);
    ...
    const after = await must<Session>("PATCH", `/__mock/rooms/${s.id}/legacy`, { body: { completion_condition: { op: "and", conditions: [{ type: "artifact_submitted", who: "assignee" }, { type: "agent_approval", agent_id: lead.id }] } } });
  ...
  it("검증은 createSession 과 같다 — 리뷰어 없으면 422 reviewer_required, 참여자 아니면 reviewer_not_participant", async () => {
    const s = await activeSession();
    const r1 = await call("PATCH", `/__mock/rooms/${s.id}/legacy`, { body: { completion_condition: { op: "and", conditions: [{ type: "agent_approval" }] } } });
    expect(r1.status).toBe(422);
    expect((r1.body as { errors: { code: string }[] }).errors[0].code).toBe("reviewer_required");
    const r2 = await call("PATCH", `/__mock/rooms/${s.id}/legacy`, { body: { completion_condition: { op: "and", conditions: [{ type: "agent_approval", agent_id: "00000000-0000-0000-0000-000000000000" }] } } });
    expect((r2.body as { errors: { code: string }[] }).errors[0].code).toBe("reviewer_not_participant");
  });

  it("Director 가 아니면 403 director_required(서버 updateWork 문장 — 옛 「Director 권한이 필요합니다」는 R4 에서 지워졌다)", async () => {
    const s = await activeSession();
    await login("seoyeon@colab.dev");
    const res = await call("PATCH", `/__mock/rooms/${s.id}/legacy`, { body: { completion_condition: { op: "and", conditions: [{ type: "manual" }] } } });
    expect(res.status).toBe(403);
    expect(res.body).toMatchObject({ code: "director_required", detail: W.work_director_required });
  });

  it("끝난 미션의 조건 고치기는 updateWork 가 409 work_closed 로 막는다(옛 updateSession 의 422 immutable 은 R4 에서 op 과 함께 지워졌다)", async () => {
    const s = await activeSession();
    await must("POST", `/works/${s.id}/cancel`, { body: { reason: "x" } });
    const res = await call("PATCH", `/works/${s.id}`, { body: { completion_condition: { op: "and", conditions: [{ type: "manual" }] } } });
    expect(res.status).toBe(409);
    expect(res.body).toMatchObject({ code: "work_closed" });
  });

  it("artifact_submitted 의 지정 제출자가 참여자가 아니어도 같은 코드(reviewer_not_participant) · 제출자 문장", async () => {
    const s = await activeSession();
    const res = await call("PATCH", `/__mock/rooms/${s.id}/legacy`, { body: { completion_condition: { op: "and", conditions: [{ type: "artifact_submitted", agent_id: "00000000-0000-0000-0000-000000000000" }, { type: "user_approval" }] } } });
    expect(res.status).toBe(422);
  ...
  it("goal·한도만 바꾸면 진행률 이벤트는 없다(work.updated 만 — 옛 session.updated 는 R4 에서 지워졌다)", async () => {
    const s = await activeSession();
    const frames = listen(s.workspace_id);
    const next = await must<Session>("PATCH", `/__mock/rooms/${s.id}/legacy`, { body: { goal: "새 목표", limits: { budget_usd: 50 } } });
    expect(next.goal).toBe("새 목표");
    expect(next.limits.budget_usd).toBe(50);
    expect(next.limits.time_limit).toBe("PT4H"); // 부분 갱신 — 다른 키 보존
    expect(frames.some((f) => f.type === "work.completion_progress")).toBe(false);
    expect(frames.some((f) => f.type === "work.updated")).toBe(true);
    expect(frames.some((f) => f.type.startsWith("session."))).toBe(false);
  });
});
...
describe("__mock seed-legacy-condition — 리뷰어 없는 옛 세션", () => {
  it("agent_approval 에 리뷰어가 없어 blocked_reason reviewer_missing · next_actor 없음 · 총 2", async () => {
    const res = await create(undefined);
    const s = res.body as Session;
    const seeded = await must<Session>("POST", `/__mock/rooms/${s.id}/seed-legacy-condition`, { body: {} });
