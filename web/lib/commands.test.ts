 ...
 * 못 잡는다 — T-C5 교훈). 그리고 `ColabCommand` ↔ 사람 말 표(`COMMAND_LABEL`)가 계약 enum 13개 **전부**를 덮는지, 요약 문장이 6역할에서 어떻게
 * 나오는지 잰다.
 */
...
describe("§2.5 표 — 웹 표와 목 표가 각각 계약 원문과 같다", () => {
  ...
  it("계약 enum 은 13개이고 ALL_COMMANDS 가 같은 순서다", () => {
    expect(ENUM).toHaveLength(13);
  ...
  it("lead·custom 은 전부, 실무자 셋은 9개(위임·검토 승인/반려·완료 승인 요청 없음), reviewer 는 10개(위임·아티팩트 제출·완료 승인 요청 없음)", () => {
    ...
    for (const r of ["researcher", "writer", "engineer"] as const) {
      expect(commandsForRole(r)).toHaveLength(9);
    ...
    expect(commandsForRole("reviewer")).toHaveLength(10);
...
describe("ColabCommand ↔ 사람 말 — 13개 전부", () => {
...
describe("summarizeCommands — 역할 6종의 요약 문장", () => {
  ...
  it("목록이 비면(옛 서버·daemon-protocol '비면 전부') 전부로 본다 · 서버 값이 표와 달라도 서버 값을 그린다", () => {
    ...
    expect(s.denied).toHaveLength(12);
  });
});
