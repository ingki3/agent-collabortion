...
# ── 방 + 미션 하나(목 시드 — 옛 createSession 모양, v0.3.0 R4 에서 op 은 지워졌다. 방 id = 미션 id) ──
...
# ── HITL 카드(S7) ──────────────────────────────────────────────────────────
H=$(curl -sS -b "$J" -X POST "$B/__mock/rooms/$SID/seed-hitl" -H 'content-type: application/json' -d '{}')
...
LH=$(curl -sS -b "$J" "$B/rooms/$SID/hitl-requests")
...
HBMSG=$(curl -sS -b "$J" "$B/rooms/$SID/messages")
...
chk "task 범위 예산 HITL 은 미션을 멈추지 않는다 (E9-01)" "$(curl -sS -b "$J" "$B/works/$SID" | py 'import sys,json;print(json.load(sys.stdin)["status"])')" "active"
...
curl -sS -b "$J" -X POST "$B/__mock/rooms/$SID/seed-hitl" -H 'content-type: application/json' -d '{"age_ms":108000000}' -o /dev/null
...
# ── 상단 액션: 참여자 · Director 교체 — 옛 세션 op(listParticipants·removeParticipant·changeDirector·pauseSession)은 v0.3.0(R4)에서
#    지워졌다. 방 참여자(listRoomParticipants)와 미션 op(/works/*)으로 같은 것을 잰다(옛 「assignee 제거 409」는 방 참여자 op 에 없다).
PARTS=$(curl -sS -b "$J" "$B/rooms/$SID/participants")
chk "에이전트 참여자 2명" "$(echo "$PARTS" | py 'import sys,json;print(len([p for p in json.load(sys.stdin)["items"] if p["kind"]=="agent"]))')" "2"
UID2=$(curl -sS -b "$J" "$B/workspaces/$WS/members" | py 'import sys,json;d=json.load(sys.stdin)["items"];print([m["user"]["id"] for m in d][1])')
CD=$(curl -sS -b "$J" -X PUT "$B/works/$SID/director" -H 'content-type: application/json' -d "{\"director_user_id\":\"$UID2\"}")
chk "Director 교체 → my_work_role 이 member 로" "$(echo "$CD" | py 'import sys,json;print(json.load(sys.stdin)["my_work_role"])')" "member"
NOAUTH=$(curl -sS -b "$J" -o /dev/null -w '%{http_code}' -X POST "$B/works/$SID/pause")
chk "일반 멤버의 일시정지는 403 (S7-P)" "$NOAUTH" "403"
[ "$MOCK" = "1" ] && curl -sS -b "$J" -X POST "$B/__mock/rooms/$SID/role" -H 'content-type: application/json' -d '{"role":"director"}' -o /dev/null

# ── paused 배너 5종(목 시드) — 재개는 resumeWork. 옛 「runtime_offline 은 재개 불가 409」는 방 멈춤(room_blocked)으로 옮겨 갔다(시드는 방을 멈추지 않는다).
if [ "$MOCK" = "1" ]; then
  for reason in budget time loop runtime_offline director; do
    P=$(curl -sS -b "$J" -X POST "$B/__mock/rooms/$SID/pause" -H 'content-type: application/json' -d "{\"reason\":\"$reason\"}")
    chk "paused($reason) detail.reason" "$(echo "$P" | py 'import sys,json;print(json.load(sys.stdin)["paused_detail"]["reason"])')" "$reason"
    RS=$(curl -sS -b "$J" -o /dev/null -w '%{http_code}' -X POST "$B/works/$SID/resume" -H 'content-type: application/json' -d '{}')
    chk "  resumeWork 200" "$RS" "200"
  done
fi

# ── 종료(completeWork) — 진행 중 서브 미션이 있으면 confirm 필요(계약 v0.2.4) ──────────
[ "$MOCK" = "1" ] && curl -sS -b "$J" -X POST "$B/__mock/rooms/$SID/seed-lanes" -H 'content-type: application/json' -d '{}' -o /dev/null
NOCONF=$(curl -sS -b "$J" -o /dev/null -w '%{http_code}' -X POST "$B/works/$SID/complete" -H 'content-type: application/json' -d '{}')
chk "진행 중 서브 미션 + confirm 없음 → 409 running_lanes" "$NOCONF" "409"
CP=$(curl -sS -b "$J" -X POST "$B/works/$SID/complete" -H 'content-type: application/json' -d '{"confirm":true}')
chk "confirm → completed" "$(echo "$CP" | py 'import sys,json;print(json.load(sys.stdin)["status"])')" "completed"
