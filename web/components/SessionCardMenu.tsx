 ...
 *
 * 키보드: 버튼에서 Enter·Space·↓ 로 열고 첫 항목에 초점, ↑↓ 로 항목 이동, Esc 로 닫고 버튼에 초점. 바깥 클릭·초점 이탈로 닫는다.
 */
...
export function SessionCardMenu({ href, gate, onDelete, testId }: SessionCardMenuProps) {
  ...
  // 바깥 클릭·초점 이탈 — document 에서 듣는다(카드 링크를 눌러 이동해도 남지 않게).
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div className="card-menu" ref={root} data-testid={testId ? `session-menu-${testId}` : "session-menu"}>
      <button
        ref={button}
        type="button"
