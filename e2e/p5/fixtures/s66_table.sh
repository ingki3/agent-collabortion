...
S="$1"; DLOG="$2"; LABEL="${3:-}"
printf 'label\tagent\ttask.attempt\toutcome\tfailure_kind\tturn_s\tmax_gap_s\tlast_event\tlast_event_at\tstall_line\n'
psqlq "select a.name, t.id, ta.attempt, coalesce(ta.outcome,'-'), coalesce(ta.failure_kind::text,'-'),
              ...
              -- 서버가 받은 이벤트 사이의 최대 공백(s) — stall 워처가 보는 "무음" 의 상한 근사(#204 이전엔 session/update 만 셌다)
              coalesce((select round(max(extract(epoch from (x.t - x.prev)))::numeric,1)::text from
                        (select coalesce(e.ts,e.created_at) t, lag(coalesce(e.ts,e.created_at)) over (order by e.seq) prev
                           from task_event e where e.task_id=t.id and e.attempt=ta.attempt and e.seq < 1073741824) x),'-'),
       ...
       where t.session_id='$S' order by ta.started_at" | while IFS=$'\t' read -r agent task attempt outcome kind turn_s gap ev ev_at; do
  stall="$({ grep -F "$task.$attempt" "$DLOG" 2>/dev/null | grep -i "stall" | tail -1 | cut -c1-160 || true; } | tr '\t' ' ')"
  printf '%s\t%s\t%s.%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$LABEL" "$agent" "${task:0:8}" "$attempt" "$outcome" "$kind" "$turn_s" "$gap" "$ev" "$ev_at" "${stall:--}"
done
