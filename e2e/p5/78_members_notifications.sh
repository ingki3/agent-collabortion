...
chk B.8 "$OWNER_UID" "$(psqlq "select w.director_user_id from room r join work w on w.id = r.legacy_work_id where r.id='$SID'")" "  … 그 방의 방장(세션을 만든 owner)이 Director 를 이었다"
