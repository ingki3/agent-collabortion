...
| `01_vertical_slice.sh` | (a) 가입→워크스페이스→페어링→`daemon pair`→probe→Lead(haiku)→세션→`@Lead 인사해줘`×N, claim·첫 출력·답글 지연 중앙값 (E17-01·02). **5b**: 긴 답변 1회 + 세션 SSE 구독으로 C-1(heartbeat 422 0 / 재큐잉 0 / `message.delta` ≥1) | `a-latency.tsv` `a-summary.json` `a-ids.txt` `a-sse.txt` `daemon-a.log` |
...
| `04_u1_browser.sh` | (d) agent-browser 로 EVAL_USER U1 1~13 + U13 실서버, 단계별 "보이는 것" 판정. U1-3a 앞에서 **같은 이름 워크스페이스를 하나 먼저 만들어** 브라우저가 만드는 것이 두 번째가 되게 한다(S-6 회귀). `U1_STOP_AFTER=4` 로 두면 1~4단계만(에이전트 턴 0) | `d-steps.tsv` `d-summary.json`, `web/__screenshots__/p1-u1-0*.png` · `web/__screenshots__/v018/p1-u1-*.png`(v0.18 세션 화면 — T-R2-M8 에서 이동) |
