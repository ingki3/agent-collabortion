...
| `04_u1_browser.sh` | (d) agent-browser 로 EVAL_USER U1 1~13 + U13 실서버, 단계별 "보이는 것" 판정. U1-3a 앞에서 **같은 이름 워크스페이스를 하나 먼저 만들어** 브라우저가 만드는 것이 두 번째가 되게 한다(S-6 회귀). `U1_STOP_AFTER=4` 로 두면 1~4단계만(에이전트 턴 0) | `d-steps.tsv` `d-summary.json`, `web/__screenshots__/p1-u1-*.png` |
