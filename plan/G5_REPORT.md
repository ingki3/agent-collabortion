...
| 확인 | 기대 | 실측 |
|---|---|---|
| 자식 lane 상태 | `blocked` | **`blocked`** (세션 시작 24초 뒤) |
...
| 위임자 기상 | **즉시** 시스템 메시지 task 1개 | **1개** — 그 시점 종료된 형제 lane 1개(형제 상태와 무관하게 성립) |
...
**여기서 한 가지가 드러났다(S-25 두 번째 절반).** 첫 실행에서 Lead 는 지시대로 **멘션 없이**
