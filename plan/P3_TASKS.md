...
| 항목 | 내용 |
|---|---|
| 상태 | **P3b 완료 → T-I3 착수(2026-09-07).** 머지: 골든 #108·#113·#120, 스파이크 4c #117, 데몬 #121·#129, CLI #126, 웹 #130, **서버 #124**(Hermes 3라운드: R1 재개 프롬프트 HITL 답변·R2 gc v0.7·S-43). 계약: #110·#116·#119·#122·#123·#127·#128. 흡수 백로그 해결 표기. **T-I3(G6 판정 자료) 진행 중** — 시나리오 C·D, HITL 왕복, 중복 0, 예산, deputy → `plan/G6_REPORT.md` → `plan/G6_DECISION.md`(컷 3) |
...
16. **e2e 지시문 기본 문구(2026-09-07, G6 §8 되먹임 1·#138 리뷰 NN1)**: 에이전트 지시문에 "저장소나 다른 디렉토리를 뒤지지 마라"와 "도구가 실패해도 재시도하거나 다른 방법을 찾지 마라"를 넣는다. X-2 의 변종은 goal 이 아니라 **도구 실패 뒤 탐색**에서 난다(`colab_hitl_ask` 404 → 30여 툴콜로 저장소 탐색).
17. **`e2e/p3/` 스크립트 번호는 Lead 가 준다**(#136·#142 리뷰 NN2 — `41_`·`48_` 이 스트림마다 겹쳤다): 40~47 스트림 실서버 스모크(CLI 40·41, 서버 41·48 은 기존대로 두되 이후 신규는 54_ 부터), 48~53 T-I3 G6 판정 스크립트, 54+ 는 Lead 배정. 같은 번호 재사용 금지, 산출물 `out/` 파일명도 스크립트 번호를 따른다.
