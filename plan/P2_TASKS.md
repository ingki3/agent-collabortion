...
4. **T-I2 1부** → **G4 판정**(P2a 초록 + 시나리오 A 단일). 미달이면 Hermes 어댑터를 P3로 이월.
   ...
   **착수 2026-09-06**(`task_0bec4deae960`, 워크트리 p1-integration, 브랜치 test/g4-scenario-a). 판정 수치: Lead 기상 = 정확히 3(위임 1 + 합류 1 + 통보 1), lane 3 동시 running 구간, 다운로드 바이트 = Content-Length, 웹(agent-browser)과 API/CLI 양쪽.
