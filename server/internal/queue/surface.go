...
var shellSurface = Surface{
	...
	ArtifactsHeader: "Artifacts submitted in this session (read one with `colab artifact get <name>`):\n",
	ReuseArtifacts:  "이전 세션의 아티팩트 %d개 — `colab artifact get <name>` 로 읽어라.\n",
...
var mcpSurface = Surface{
	...
	ArtifactsHeader: "Artifacts submitted in this session (read one with the `colab_artifact_get` tool):\n",
	ReuseArtifacts:  "이전 세션의 아티팩트 %d개 — `colab_artifact_get` 툴로 읽어라.\n",
