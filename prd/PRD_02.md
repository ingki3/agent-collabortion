...
| 항목 | 내용 |
|---|---|
| 문서 버전 | **v0.5 (Draft)** — 리뷰 #01 반영. 이전: v0.4 (`PRD_ANTHROPIC.md`) |
| 작성일 | 2026-09-02 |
| 리뷰 이력 | `PRD_REVIEW_01.md` (Critical 6 / Major 12 / Minor 15) — 전건 반영. C2·C3은 원안 대신 수정안 채택(아래) |
...
| C2 | `steer`(기본) = 취소 후 재프롬프트. 런타임 `steering` 능력에 의존 | **정책명 `restart-with-context`로 변경, 기본값은 `queue`.** 취소는 **명시 멘션에만** 적용(작성자 무관), 암묵 라우팅은 항상 큐잉. Director의 중단은 메시지가 아니라 UI 버튼. `steering` 능력은 인터페이스에서 제거 | FR-3.4, §8.2.1 |
| C3 | lane = 위임마다 하나. 작업 공간도 lane마다 | **lane(동시성 단위)과 workspace(작업 공간)를 분리.** workspace 바인딩은 격리 방식이 정한다: `worktree`는 에이전트당 1개 공유, `container`·`none`은 lane당 1개 | FR-6.1, FR-6.4, §7 |
...
| **Workspace** | 실제 작업 디렉토리. lane과 **분리된 개념**이며, 격리 방식이 바인딩을 정한다 — `worktree`는 에이전트당 하나를 공유하고, `container`·`none`은 lane당 하나(FR-6.1). |
...
4. Lead가 자동 트리거 → 계획을 메시지로 올리고 `@Researcher`에게 3개 조사 항목을 병렬 위임. **lane 3개가 생기고 격리가 `none`이므로 workspace도 3개**(FR-6.1).
...
3. PM이 스펙 작성 → `@Backend` `@Frontend` 병렬 위임 → lane 2개. **격리가 워크트리이므로 workspace는 에이전트당 하나** — `colab/<session>/backend`, `colab/<session>/frontend` 브랜치.
4. 각자 구현 후 `colab artifact submit --type diff`로 diff를 제출한다. **QA는 남의 워크트리에 접근하지 않고 아티팩트만 본다**(FR-6.1).
5. QA가 Frontend에 수정 요청 멘션 → **Frontend의 기존 lane이 `done → running`으로 재진입**해 같은 워크트리에서 이어 작업 → 새 diff 제출 → QA 승인.
6. 세션 `completed`. 워크트리는 14일 보존되며 병합은 Director가 수행한다. 미병합 커밋이 있으면 GC가 삭제하지 않는다(FR-6.4).

### 시나리오 C — Director 중간 개입

- Director가 "범위를 한국 시장으로 좁혀줘"라고 **멘션 없이** 쓰면 규칙 6으로 assignee(Lead)에게 가고, Lead가 실행 중이면 **큐잉된다**. 진행 중인 턴은 죽지 않는다.
- 지금 당장 멈춰야 하면 세션 화면의 **"지금 중단하고 다시 지시" 버튼**을 쓴다. 이것이 `interrupt`를 수행한다(FR-3.4).
- 특정 에이전트에게 명시 멘션으로 지시하면 세션의 `mid_turn_policy`가 적용된다.
...
**FR-1.3 Agent 상태** — `idle` / `working` / `waiting_human` / `error` / `offline`(세션 런타임 연결 끊김) / `disabled`(`respond_to: nobody`로 사람이 정지시킴). 세션 화면의 참여자 목록과 에이전트 페이지에 실시간 표시. `offline`과 `disabled`는 원인이 다르므로 구분해 표시한다.
...
- `nobody`는 폭주하는 에이전트를 즉시 멈출 때 쓴다. 에이전트 상태에 `disabled`로 표시되며 `offline`(런타임 연결 끊김)과 구분한다.
...
| `runtime_id` | **실행 머신 (C4).** `isolation`이 `worktree`·`container`면 **필수**, `none`이면 선택(비우면 온라인 런타임 중 자동). 세션의 모든 lane이 이 머신에서 실행된다. |
| `participants` | 참여 에이전트 목록 + 각자의 프로파일 + 그중 `assignee`(기본 담당, 보통 lead) |
| `context` | 첨부 자료: 문서, URL, 파일, 이전 세션 링크 |
| `isolation` | 작업 공간 격리 방식 (필수): `worktree` (저장소 경로 지정) / `container` (이미지 선택) / `none`. 격리 방식이 workspace 바인딩 규칙을 정한다(FR-6.4) |
| `completion` | 종료 조건 (FR-2.2) |
| `limits` | 예산 상한(USD/토큰), 시간 상한, 최대 task 수, 최대 병렬 lane 수. **초과는 종료가 아니라 `paused`** (FR-2.2 아래 설명) |
| `mid_turn_policy` | 실행 중 새 메시지 처리: `queue`(기본) / `restart-with-context` / `interrupt` (FR-3.4) |
...
7. 규칙 5로 비-assignee가 트리거된 경우, assignee 폴백 task를 5분 지연 예약하고 주 에이전트가 응답하면 취소. **지연 폴백은 발화 시점에 assignee가 실행 중이면 항상 큐잉된다** — 지연 폴백이 진행 중인 턴을 취소하는 일은 없다(FR-3.4의 정책과 무관하게 적용).

**FR-3.4 턴 진행 중 도착한 메시지 처리** — 에이전트가 이미 실행 중일 때 그 에이전트를 향한 새 메시지가 오면 다음 규칙으로 처리한다.

**판별자는 하나다: 명시 멘션인가 아닌가.** 작성자가 Director인지 여부로 나누지 않는다. Director의 "좋아요, 계속해"는 규칙 6의 암묵 라우팅으로 assignee에게 가는데, 작성자로 판별하면 바로 그 가장 흔한 상호작용이 진행 중인 턴을 죽인다.

| 유입 경로 | 동작 |
|---|---|
| **암묵 라우팅** (규칙 5·6·7로 도달) | **항상 `queue`.** 예외 없음 |
| **명시 멘션** (`[@Agent](mention://agent/…)`) | 세션 `mid_turn_policy`를 적용 |

`mid_turn_policy` (명시 멘션에만 적용):

| 정책 | 동작 | 적합 |
|---|---|---|
| `queue` **(기본)** | 현재 턴을 끝까지 두고, 새 메시지는 다음 run으로 병합해 대기시킨다(`coalesced_message_ids`). | 대부분의 세션 |
| `restart-with-context` | 진행 중인 턴을 취소하고, **이전 지시와 새 메시지를 함께** 다시 프롬프트한다. 새 메시지는 "작업 도중 도착함"으로 프레이밍한다(§8.4의 `prior`/`new` 구간). | 방향 전환이 잦은 탐색적 세션 |
| `interrupt` | 진행 중인 턴을 취소하고 **새 메시지만으로** 다시 시작한다. 이전 지시는 히스토리에만 남는다. | 명백한 오작동 중단 |

- **Director의 즉시 중단은 메시지가 아니라 UI 버튼으로 한다.** 세션 화면의 "지금 중단하고 다시 지시" 버튼이 `interrupt`를 수행한다. 의도가 분명한 조작에만 파괴적 동작을 붙인다.
- **되돌리기 어려운 작업 중에는 취소를 보류한다.** 판정 기준: 마지막 `task_event`가 파일 편집 또는 셸 명령이고 그 완료 이벤트가 아직 오지 않았으면, 완료를 기다렸다가 취소한다. 최대 대기 30초를 넘기면 그대로 취소하고 그 사실을 활동 피드에 남긴다.
- 취소는 §8.2.2의 절차(권한 요청 응답 → `session/cancel` → 드레인)를 따른다. 프로세스를 즉시 죽이지 않는다.
- 정책과 무관하게, **같은 에이전트에 대한 `queued` 상태의 task는 항상 하나로 병합**한다. 병합된 메시지 id는 `coalesced_message_ids`에 보존한다.
- lane 하나당 진행 중인 턴은 항상 1개다(FR-6.3). 여러 lane은 동시에 진행된다.

> v0.4는 이 정책을 `steer`라 부르고 런타임의 `steering`(진행 중 턴에 메시지 **주입**) 능력에 연동했다. 취소 후 재프롬프트는 `session/cancel`만 있으면 되므로 그 연동은 근거가 없었다. v0.5에서 이름을 `restart-with-context`로 바꾸고 `steering` 능력을 인터페이스에서 제거했다. 진짜 주입 방식은 그 능력을 가진 런타임이 확인되면 별도 정책으로 추가한다.
...
- **서버측 억제**: 본문이 짧고 멘션만 있으며 진행·결과·질문 신호가 없는 메시지는 게시는 하되 **트리거하지 않는다**. §8.3의 "빈 확인 메시지 금지"는 프롬프트 규칙이라 어기는 순간 무한 루프가 되므로, 서버에도 같은 규칙을 둔다. 오탐 시 에이전트가 다시 멘션하면 되므로 손실은 작다.
...
- task는 `waiting_human`, 에이전트 상태도 `waiting_human`. 세션 타임라인에 카드로 게시.
...
- `due_in` 지정 가능(기본 24시간). 기한 초과 시 (a) 대기 유지 (b) `proposed_default`로 진행 — 세션 `autonomy` 설정에 따름.
...
- **응답 = 새 attempt.** 재개는 다음 순서로 시도한다.
  1. **런타임 세션 resume** — `runtime_session_ref`(런타임 종류 + 세션 id + provenance)로 이전 세션을 이어받고, "질문 / 답변 / 승인 여부 / 사유"를 담은 턴 프롬프트를 보낸다.
...
**FR-6.1 Lane과 Workspace의 분리** — v0.4는 lane 하나가 "동시성 단위"와 "작업 공간"을 겸했다. 그 결과 같은 에이전트에게 항목 3개를 병렬 위임하면(시나리오 A) 워크트리가 3개 생기고, 리뷰어가 남의 워크트리를 어떻게 보는지도 정의되지 않았다. 두 개념을 분리한다.

| 개념 | 정의 | 수명 |
|---|---|---|
| **Lane** | 실행 트랙. 위임 하나가 lane 하나를 만든다. 같은 에이전트가 여러 lane을 동시에 가질 수 있다. 동시성·의존성(DAG)·보드 표시의 단위 | 위임 → 완료 |
| **Workspace** | 실제 작업 디렉토리. 격리 방식이 바인딩 규칙을 정한다(FR-6.4) | 세션 종료 후 GC까지 |
...
- **재위임은 기존 lane을 재사용한다.** 이미 `done`인 에이전트에게 수정을 요청하면(시나리오 B의 "Frontend에 수정 요청") 그 에이전트의 **기존 lane이 `done → running`으로 재진입**하고 같은 workspace에서 이어 작업한다. 새 lane과 새 워크트리를 만들지 않는다 — 이전 수정이 유실되기 때문이다.
...
**FR-6.2 의존성** — `colab lane delegate --agent --brief --depends-on <lane_id> --profile <name>` 으로 DAG를 구성. 의존 lane이 완료되면 자동 시작. UI에 lane 보드(칸반) 제공: `queued / running / waiting_human / done / failed`. lane 상태 머신은 `done → running` 재진입을 허용한다(위 재위임 규칙).

**FR-6.3 동시성 제한** — 세션 `max_parallel_lanes`(기본 5), 에이전트 `max_concurrent_tasks`, 런타임(데몬) 전역 상한, 워크스페이스 전역 상한. 초과분은 큐 대기. `waiting_human` 상태는 어느 계산에도 포함하지 않는다(FR-5.4). **`worktree` 격리에서는 같은 에이전트의 lane들이 workspace를 공유하므로 항상 순차 실행된다.**

**FR-6.4 격리와 작업 공간 정리** — 세션 생성 시 선택한 `isolation`에 따라 데몬이 workspace를 준비한다.
...
- `worktree`: 세션 종료 후에도 **기본 14일 보존**(워크스페이스 설정 `workspace_retention_days`). 병합·정리는 Director가 수행한다. 보존 기한이 지나면 **병합되었거나 커밋이 없는** 워크트리만 자동 삭제하고, 미병합 커밋이 있으면 삭제하지 않고 Director에게 알린다.
- 데몬은 워크스페이스 루트의 디스크 사용량을 보고하고, 워크스페이스 설정의 상한(`workspace_disk_quota_gb`)에 도달하면 새 세션 생성을 막고 Director에게 정리를 요청한다.
- Runtimes 화면에 머신별 작업 공간 목록·용량·마지막 사용 시각을 표시하고 수동 삭제를 제공한다.

**FR-6.5 합류(Join)** — 위임자에게 결과를 전달하는 **유일한 경로**다. 자식 lane이 개별적으로 위임자를 깨우지 않는다(FR-3.3 규칙 4).

- 모든 자식 lane이 종료 상태(`done` 또는 `failed`)가 되면 위임자 에이전트에 `progress_update` 시스템 메시지로 결과 묶음이 **한 번** 전달되어 종합 run이 트리거된다.
- **일부가 실패해도 나머지를 기다린다.** 실패한 lane은 실패 사유와 함께 묶음에 포함된다. 위임자가 재시도·우회·에스컬레이션을 판단한다.
- 자식이 `waiting_human`이면 종료 상태가 아니므로 합류는 대기한다. 그 대기가 세션 시간 상한에 걸리면 `paused`가 되고 Director에게 알린다.
...
- `preparing`: 데몬이 workspace를 준비하는 단계.
- `dispatched` 5분 초과 → `failed(timeout)`. `running`은 15초 heartbeat, 3분 무응답 시 런타임 offline 처리.
- `waiting_human`은 HITL 요청으로 진입하고(FR-5.4) 응답·기한 만료로 나온다. **이 상태에서는 heartbeat를 요구하지 않으며 동시성 슬롯도 점유하지 않는다.** 프로세스는 이미 종료되어 있다.
...
1. **같은 workspace를 재사용한다.** 새로 만들지 않는다.
2. **런타임 세션 resume을 먼저 시도**하고, 불가하면 콜드 스타트로 폴백한다.
3. `attempt ≥ 2`의 턴 프롬프트에 다음을 주입한다: 이전 시도가 중단되었다는 사실, **이미 게시한 메시지 목록**, "작업을 이어가기 전에 workspace의 현재 상태(변경된 파일, git status)를 먼저 확인하라"는 지시.
...
- **턴 중 강제**: 데몬이 `usage_update`(ACP) 또는 각 이벤트의 사용량을 누적해, `budget_per_task` 또는 세션 잔여 예산을 넘으면 §8.2.2의 취소 절차를 밟고 `failed(budget_exceeded)`로 보고한다.
...
| **Sessions** | 목록 (상태·goal·Director·비용·참여자 아바타), 새 세션 마법사 (goal → Director → **런타임** → 참여자·프로파일 → 격리 → 종료 조건 → 한도) |
| **Session 상세** | 좌: 참여자(프로파일 표시) + lane 보드 / 중: 메시지 타임라인(스레드, HITL 카드, 활동 피드 펼치기) + **"지금 중단하고 다시 지시" 버튼** / 우: goal·종료조건 진행률·아티팩트·결정 기록·비용 |
...
workspace 1─N agent (name, role, role_description, instructions, tools,
                     owner_id, permission, respond_to, respond_to_allowlist[],
...
session   1─N workspace_dir (agent_id?, lane_id?, kind: worktree|container|dir,
...
workspace 1─1 workspace_settings (loop_limits, budget_policy, context_reuse,
                                  ...
                                  workspace_retention_days, workspace_disk_quota_gb)
...
- **`workspace_dir`가 lane과 작업 공간을 분리한다(C3).** `worktree` 격리에서는 `agent_id`만 채워 같은 에이전트의 lane들이 하나의 행을 공유하고, `container`·`none`에서는 `lane_id`를 채워 lane마다 하나씩 갖는다. GC는 이 테이블의 `retain_until`·`disk_bytes`를 기준으로 동작한다(FR-6.4).
...
- **`task.runtime_session_ref`가 재개의 유일한 근거다(C1).** HITL 응답 후 재개와 실패 후 재시도가 같은 필드를 쓴다. `provenance`는 재개 시 세션 동일성 검증에 쓴다(§8.2.5의 Hermes 유실 감지).
...
interface RuntimeHarness {
  ...
  start(workspace, profile): SessionHandle      // ACP: initialize + session/new|resume
  prompt(handle, turn): AsyncIterable<TaskEvent>
  cancel(handle)                                // FR-3.4의 취소·재프롬프트가 이것만 쓴다
}
```

`steering`(진행 중 턴에 메시지 주입) 능력과 `steer()` 메서드는 v0.5에서 제거했다. FR-3.4의 정책은 취소 후 재프롬프트이므로 `cancel()`만 있으면 되고, 실제 주입을 지원하는 런타임이 확인되지 않았다.
...
**지시 파일이 워크트리를 오염시키지 않게 한다 (M6)** — `worktree` 격리에서 workspace는 저장소 체크아웃 그 자체다. 여기에 `CLAUDE.md`/`AGENTS.md`를 그냥 쓰면 (a) 저장소가 원래 갖고 있던 같은 이름 파일을 덮어써 프로젝트 규칙이 사라지고, (b) `git status`에 잡혀 에이전트가 커밋해버린다.

- 원본 파일이 있으면 **덮어쓰지 않고**, 구분 마커 사이에 우리 브리프를 **덧붙인다**. lane 종료 시 마커 구간을 제거해 원본을 복원한다.
- 브리프 파일 경로를 `.git/info/exclude`에 등록해 `git status`에 나타나지 않게 한다(`.gitignore`는 저장소 파일이므로 건드리지 않는다).
- 원본이 없었으면 lane 종료 시 파일을 삭제한다.

**턴 프롬프트**

- 트리거 메시지 인용, 위임 브리프, 최근 히스토리 N개, "응답은 `colab message post`로 게시하라"는 지시만 담는다.
- **`restart-with-context`로 재프롬프트할 때**(FR-3.4)는 이전 지시와 새 메시지를 각각 `<prior>`, `<new>` 구간으로 나누고, 새 메시지가 작업 도중 도착했음을 명시한다. 그냥 이어 붙이면 에이전트가 두 지시를 같은 시점의 요구로 읽고 충돌한다.
- **재개·재시도 프롬프트**(FR-5.4, FR-7.1)에는 `<resumed>` 구간을 두고 중단 사유, 이미 게시한 메시지 목록, HITL 답변과 승인 여부, "workspace의 현재 상태를 먼저 확인하라"는 지시를 담는다.
...
- [ ] 세션 생성(goal, **Director**, `runtime_id`, 참여자+프로파일, **격리 worktree/none**, 종료 조건 3종, 예산 상한, `mid_turn_policy`)
- [ ] 메시지·스레드·멘션 라우팅(FR-3.3 규칙 1~7), 트리거 병합, **명시 멘션에만 적용되는 취소 정책**
- [ ] `colab` CLI + MCP 서버, 브리프 파일 생성(워크트리 오염 방지 포함), 비용 집계 + 턴 중 예산 강제
- [ ] **활동 피드** — 중추 5개 렌더 클래스 + 일반 툴 + 원본 레일, 제자리 갱신, 침묵 상태 표시
- [ ] HITL `question` / `approval` (**턴 종료 + 새 attempt 재개**, Director 수신), 인박스 "Needs Action" 질의
- [ ] 병렬 lane(의존성 없음), lane/workspace 분리, lane 보드, 재개·재시도 공통 경로
- [ ] 작업 공간 GC(보존 기한·용량 상한)
...
- [ ] HITL `choice` / `info`, deputy director
...
| **재시도가 부분 실행을 중복 적용** | 같은 workspace 재사용 + resume 우선 + `attempt ≥ 2` 프롬프트에 "이미 한 일" 주입(FR-7.1). 성공 지표로 중복 발생률 추적 |
...
| 에이전트 간 무한 핑퐁 | FR-3.5의 체인 깊이·시간당 빈도·연속 왕복 3중 상한 + **서버측 빈 확인 메시지 억제**. 프롬프트 규칙만으로는 막지 못한다 |
...
| Director 부재로 세션 정체 | deputy director(v1.1), 기한 초과 시 `proposed_default` 진행 옵션 |
| 종료 조건 판정 오류(조기 완료) | `criteria_met`은 단독 사용 금지 + 판정자를 플랫폼 LLM으로 고정 |
| 병렬 lane 간 산출물 충돌 | 격리별 workspace 바인딩(C3) + 크로스 lane 읽기는 아티팩트로만 |
...
| 취소가 되돌리기 어려운 작업을 중간에 끊음 | 기본값이 `queue`이고 취소는 명시 멘션에만 적용(FR-3.4). 파일 편집·셸 명령 진행 중이면 완료까지 최대 30초 대기 후 취소 |
| 지시 파일이 저장소를 오염 | 마커 사이 append + lane 종료 시 복원 + `.git/info/exclude` 등록(§8.4) |
...
4. **HITL 재개 경로 검증.** Claude Code ACP resume 후 답변 프롬프트로 이전 컨텍스트가 유지되는지, Hermes 유실 감지가 실제로 동작하는지, 콜드 스타트만으로 작업을 이어갈 수 있는지(C1).
5. **워크트리에 지시 파일 append/복원**이 각 런타임에서 실제로 읽히는지, `.git/info/exclude` 등록이 충분한지(M6).
6. **Claude Code CLI 플래그 재확인**: `--effort`, `--max-budget-usd`, `--append-system-prompt-file`. §8.2.4는 multica 코드에서 확인했으나 부록 C 공식 문서로 재대조.
7. (v1.1) 컨테이너 격리에서 각 CLI의 로그인 상태를 안전하게 전달하는 방법.
...
| 턴 진행 중 메시지 처리 (`multiple-event-handling`) | `crates/buzz-acp/src/config.rs`. **기본값과 판별자는 바꿨다** — Buzz는 `steer`가 기본이지만 우리는 `queue`가 기본이고 취소는 명시 멘션에만 적용한다(FR-3.4, C2) |
