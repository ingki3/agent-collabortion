...
| 항목 | 내용 |
|---|---|
| 문서 버전 | **v0.6 (Draft)** — 리뷰 #02 반영. 이전: v0.5 (`PRD_02.md`), v0.4 (`PRD_ANTHROPIC.md`) |
| 작성일 | 2026-09-03 |
| 리뷰 이력 | `PRD_REVIEW_01.md` (C6/M12/m15) → v0.5. `PRD_REVIEW_02.md` (신규 충돌 4 / Major 11 / Minor 15) → v0.6. Critical·Major 전건 반영, N2·N3·M6는 원안 대신 수정안 채택(아래) |
...
| `waiting_human`의 열린 HITL | **남긴다.** 사람이 답할 기회를 뺏지 않는다. 답이 와도 에이전트가 재개되지 않고 요청은 `open`으로 남아, 다시 활성화하면 그때 이어진다 |
| workdir | 보존한다 |
...
8. **자식 lane의 완료 보고가 위임자를 멘션해도 트리거하지 않는다.** 게시는 하되 합류 묶음(FR-6.5)에 실어 한 번만 전달한다. §8.3이 위임자 멘션을 금지하지만 프롬프트 규칙은 어겨질 수 있으므로 서버에도 같은 규칙을 둔다(N1).
...
- 같은 에이전트에 대한 `queued` task는 하나로 병합한다. 병합된 메시지 id는 `coalesced_message_ids`에 보존하고, 다음 run의 턴 프롬프트에 도착 순서대로 인용한다.
...
| 동작 | 절차 | 결과 |
|---|---|---|
| **중단하고 다시 지시** | 그 lane의 에이전트를 멘션한 메시지 작성창이 열린다. 전송하면 진행 중인 턴을 취소하고 **새 메시지만으로** 다시 시작한다 | lane은 `running` 유지, `attempt` 증가 |
...
**승인은 어떤 경우에도 자동으로 나지 않는다.** `autonomy: autonomous`에서도 마찬가지다. 침묵을 승인으로 해석하면 사람 게이트의 의미가 사라지고, 거절로 해석하면 정상 작업이 기한 때문에 죽는다. 둘 다 틀렸으므로 기다린다. 대신 기한이 지나면 상태를 `expired`로 바꾸지 않고 **`open`을 유지한 채 `overdue` 플래그**를 세운다 — 늦었어도 답할 수 있어야 하고, 인박스는 `overdue`를 맨 위로 올린다.
...
- **재위임은 기존 lane을 재사용한다.** 이미 `done`인 에이전트에게 수정을 요청하면(시나리오 B의 "Frontend에 수정 요청") 그 에이전트의 **기존 lane이 `done → running`으로 재진입**하고 같은 workdir에서 이어 작업한다. 새 lane과 새 워크트리를 만들지 않는다 — 이전 수정이 유실되기 때문이다.
...
- Director가 "계속"을 승인하면 해당 task를 `queued`로 되돌린다. 새 트리거를 기다리지 않는다.
...
- 데몬은 시작 시 지원 바이너리를 탐지하고 `probe()`로 버전·모델 목록·로그인 여부를 보고한다. 이후 60초마다 갱신. `worktree` 세션을 위해 `repo_path` 존재 여부와 git 상태도 함께 보고한다.
...
- Director는 둘 중 하나를 고른다.
  - **재바인딩**: `isolation: none`이면 다른 온라인 런타임으로 옮긴다. `worktree`·`container`면 같은 저장소·이미지를 가진 런타임으로만 옮길 수 있고, workdir은 유실되므로 진행 중이던 lane은 콜드 스타트한다. 아티팩트와 메시지는 서버에 있으므로 컨텍스트는 남는다.
...
- **"중단하고 다시 지시"로 재시작할 때**(FR-3.4)는 새 지시만 담고 이전 지시는 반복하지 않는다. **런타임 resume 위에서 재시작하면 이전 지시가 이미 런타임 세션 안에 있으므로**, 다시 넣으면 중복이 되어 에이전트가 두 지시를 같은 시점의 요구로 읽는다. 콜드 스타트일 때만 `<prior>` 구간에 이전 지시를 넣고 "이 작업은 중단되었다"고 명시한다(m-o).
...
- [ ] HITL `question` / `approval` (**턴 종료 + `turn_end` 전이 + 새 attempt 재개**, Director 수신, 승인은 자동 진행 없음), 인박스 "Needs Action" 질의
- [ ] 병렬 lane(의존성 없음), lane/workdir 분리, lane 보드 + **중단·재지시 버튼**, 합류 1회, 재개·재시도 공통 경로
- [ ] workdir GC(보존 기한·용량 상한), **고아 프로세스 정리 + 토큰 폐기**(FR-9.1)
- [ ] **런타임 영구 오프라인 시 세션 재바인딩·종료**(FR-9.2)
- [ ] 세션 자동 종료 + 요약, 컨텍스트 재사용 상한 설정

> **일정 주의**: v0.6에서 `mid_turn_policy`가 빠져 설정·UI가 줄었지만, FR-9.1(고아 프로세스)·FR-9.2(런타임 재바인딩)·M3(추적 파일 처리)가 새로 들어왔다. 셋 다 데이터 유실을 막는 항목이라 뺄 수 없으므로 **10주는 여전히 빠듯하다**. 스파이크 1이 CLI 어댑터를 v1으로 끌어들이면 재산정이 필요하다.
...
| **런타임이 영구히 사라져 세션이 고립** | 7일 오프라인이면 세션 `paused` + 재바인딩·종료 선택(FR-9.2). 활성 세션이 걸린 런타임은 삭제 차단 |
