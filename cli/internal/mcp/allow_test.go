...
func TestFilterTools(t *testing.T) {
	...
	if len(got) != 4 || got[0].Name != "colab_session_get" || got[1].Name != "colab_review_approve" || got[2].Name != "colab_review_reject" || got[3].Name != "colab_room_get" {
		names := make([]string, 0, len(got))
		for _, x := range got {
			names = append(names, x.Name)
		}
		t.Fatalf("filtered = %v, want session_get · review_approve · review_reject · room_get (session_get's alias) in table order", names)
	}
...
func TestServeAllowRegistersOnlyTheSubset(t *testing.T) {
	...
	// The reviewer's 12 commands, plus room get · room messages — the room
	// names of session get · messages, which the row allows.
	if len(names) != len(reviewer)+2 {
		t.Fatalf("tools/list = %v, want the %d reviewer tools + 2 room aliases", names, len(reviewer))
	}
	for _, n := range names {
