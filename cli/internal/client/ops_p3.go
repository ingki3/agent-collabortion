...
// CreateHitlRequest — POST /rooms/{R}/hitl-requests (createHitlRequest).
// The path is room-scoped and carries no task: the TaskToken names the
...
// changed.
func (c *Client) CreateHitlRequest(ctx context.Context, roomID string, body HitlCreate, key string) (*HitlCreateResult, error) {
	res, err := c.Do(ctx, http.MethodPost, "/rooms/"+url.PathEscape(roomID)+"/hitl-requests", nil, body, idemHeader(key))
	if err != nil {
		return nil, err
	}
