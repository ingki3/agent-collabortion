...
| 항목 | 내용 |
|---|---|
| 버전 | **v0.9.9 — 턴 중 usage 의 `model`·서버 가격 조회 순서**(Director 승인 2026-09-26, T-COSTMODEL #353 — claude_code 프로파일 model=default 인 Lead 가 턴 중 $0 으로 잡혀 예산 상한이 턴 중에 안 걸리던 결함): §7 `usage_update` 행. **v0.9.8 — `worktree` 격리 방의 `<folders>` 는 동료 줄을 싣지 않는다**(Lead 판정 2026-09-26, T-FOLDERS 구현 질문 — v0.9.7 의 「동료 체크아웃 read only」 가 E13-08(QA 번들에 동료 워크트리 경로 0건)과 충돌; E13-08 을 지킨다). **v0.9.7 — 미션 폴더(Director 승인 2026-09-26, T-FOLDERS D1~D8 · daemon-protocol v0.10.0)**: §10 턴 프롬프트에 **`<folders>` 블록** — 서버가 쓴다(경로를 서버가 짓는다, daemon-protocol §6.1). 자리는 `<roster_status>` 바로 뒤·`<trigger>` 앞(hermes `COLAB_BRIEF.md` 포인터 줄은 여전히 맨 앞). 자기 폴더(`you`)·미션 공용(`shared`)·같은 미션 동료 폴더(읽기 전용, **실제로 행이 있는 것만**)의 절대 경로 + 같은 에이전트의 병렬 lane 이 한 폴더를 쓰면 「새 파일 이름에 갈래 표지(lane id 앞 8자리)를 넣어라」(D3 A) + 마지막 문장 「다른 미션·다른 방은 아티팩트나 `colab room read`」 — **마지막 문장은 도구 표면별**(mcp 는 `colab_room_read` 툴, v0.9.6 규칙). 브리프 [2] 에 **고정 한 줄**(「네 폴더는 `<folders>` 에 — 쓰기는 네 폴더와 shared 에만」, 표면별 두 벌, 미션 유무와 무관하게 늘 같은 바이트 — [1]~[5] 바이트 동일 E12-11 유지). 쓰기 제한은 **규약만**(D5 A — `permissions.deny` 로 막지 않는다). hermes `COLAB_BRIEF.md` 는 에이전트 폴더(cwd)에 쓰고 `_shared` 에는 쓰지 않는다; `dir` 폴더는 같은 에이전트의 병렬 lane 이 함께 쓰므로 파일 이름을 lane 별로 `COLAB_BRIEF-<lane_id[:8]>.md`(worktree 는 lane 이 순차라 지금 이름 그대로). **v0.9.6 — 브리프 [2] 는 도구 표면에 맞춘 말로(Director 승인 2026-09-25)**: `tool_surface = mcp`(claude_code) 에이전트에게 서버가 만드는 **모든 텍스트**(브리프 [2]·[3] lead 조정 규약·[4] 잘림 고지, 턴 프롬프트의 아티팩트 줄·`<history>` 작업 내용 강등 줄(v0.9.4·v0.9.5 의 「`colab room messages --thread <id>` 로 전문」 포함)·마지막 지시문)는 명령을 **MCP 툴 이름과 인자**로만 적는다 — `colab_message_post`(`body`·`detail`·`detail_file`·`reply_to`·`top_level`), `colab_room_messages`, `colab_room_get`, `colab_artifact_submit` 등. 셸 명령 `colab …` 은 적지 않는다(그 환경의 셸에는 `colab` 이 없다). `tool_surface = cli_wrapper`(hermes)는 지금처럼 셸 명령으로(래퍼 경로 치환, §10). 표면별 문장은 서버가 번들의 `runtime_kind` 로 고르며 같은 에이전트·같은 표면이면 바이트 동일(E12-11). 실측: STO 이후 삼성전자 미션에서 claude_code Lead 가 브리프의 셸 문장대로 `colab message post --detail-file` 을 셸로 실행해 실패 → MCP 로 재시도(2026-09-25 12:32). **v0.9.5 — 트리거 작업 내용 상한(#333 리뷰 NN2)**: `<trigger>` 의 `<detail>` 전문은 **한 턴 합계 5만 자까지**. 병합된 트리거가 여럿이면 늦게 게시된 것부터 전문을 싣고, 합계를 넘는 메시지는 `<history>` 와 같은 모양(첫 400자 + 「(작업 내용 N자 — `colab room messages --thread <id>` 로 전문)」)으로 강등한다 — 메시지당 상한 20만 자 × 병합 N건이 한 턴 프롬프트를 채우지 않게. **v0.9.4 — 대화와 작업 내용(Director 승인 2026-09-25, PRD FR-3.1.2)**: §10 브리프 [2] 에 「`--body` 는 대화 — 누구에게·무엇을·결론·다음 할 일, 5줄 안팎. 조사 결과·초안 전문·표는 `--detail`. 제출물은 `artifact submit`」. 턴 프롬프트: `<trigger>` 의 메시지는 `detail` 전문을 `<detail>` 로 싣고, `<history>` 의 다른 메시지는 `detail` 첫 400자 + 「(작업 내용 N자 — `colab room messages --thread <id>` 로 전문)」 한 줄. **v0.9.3 — 스레드 답글(Director 승인 2026-09-25)**: §2.1 env `COLAB_THREAD_ID`(번들 `thread_root_id`, 래퍼도 export). §10 턴 프롬프트의 `<trigger>` 안 `<message>` 에 스레드 답글이면 `thread="<루트 id>"` 속성, 마지막 지시문에 「스레드로 들어온 메시지에는 그 스레드에 답한다 — `colab message post` 는 기본으로 그 스레드에 답글을 단다. 메인 타임라인에 올려야 할 때만 `--top-level`」. **v0.9.2** — §10 브리프 [5] Roster 에서 **현재 상태(working/idle)를 뺀다** — 매 턴 계산되는 값이 [5] 에 있어 [1]~[5] 바이트 동일(E12-11)이 v0.9.0 부터 깨져 있었다(#323 리뷰 재현). 상태는 **턴 프롬프트**의 `<roster_status>` 블록(참여자 한 줄씩 `- <이름>: working|idle`)으로 옮긴다. [5] 에는 이름·역할 설명·멘션 링크만. v0.9.1 — §2.1 env 에 `COLAB_ROOM_ID`·`COLAB_WORK_ID`(래퍼도 export). 브리프 [4] 에는 종료 조건 **모양**만, 진행(충족 n/m)은 턴 프롬프트 — [1]~[5] 바이트 동일을 지키려고(R3b). **v0.9.0 — 방·미션(PRD v0.19 R0)**: §10 브리프 **[4] 세션 맥락 → 방 맥락**: 방 이름·한 줄 설명 + **그 턴이 속한 미션**(goal·성공 기준·종료 조건 진행)과 방 히스토리 세 묶음(최근 N · 그 미션의 메시지 전부 · 결정 기록 전부 + 최근 「여기까지 정리」, 잘리면 한 줄 고지 — PRD FR-4.1). [1] Agent Identity 는 건드리지 않는다. [1]~[5] 바이트 동일 규칙(E12-11)은 **같은 방·같은 미션** 안에서만 성립한다 — 미션이 바뀌면 [4] 가 달라져 프리픽스 캐시가 한 번 깨진다(의도된 대가). 미션 밖 턴의 [4] 는 방 부분만. v0.8.11 — §7 D-26 답(hermes 어댑터 keepalive, 실제 출력 연동 조건). v0.8.10 — §10 MCP 툴 목록·래퍼·브리프 [2] 를 역할 부분집합으로(K-19). v0.8.9 — §7 stall 판정에 원시 스트림 이벤트를 활동으로 포함(S-66 원인: 툴 입력 생성 중 session/update 0건). v0.8.8 — §2.1 토큰 없는 번들(테스트 채팅)은 colab 표면을 전부 끈다. v0.8.7 — §10 자리표시자 치환 규칙(`{{COLAB_REBIND_DIR}}`, T-D9 PR #156 계약 결함 2). v0.8.6 — 스파이크 5(PR #153) 반영: §10 hermes 브리프는 **미추적 `COLAB_BRIEF.md` + `.git/info/exclude` + 턴 프롬프트 첫 줄 지시**(`skip-worktree` 채택 안 함 — 에이전트 커밋 0/12), §1 hermes `session/set_model` 파라미터 이름 `modelId`. v0.8.5 — §7 턴 중 usage(emitRawSDKMessages, dedup, result.total_cost_usd = 실측 비용), §3 _meta, §9 `usage_midturn`(D-17·K-8 데몬 절반). v0.8.4 — §2.2 refusal 예외: 재개 직후 활동 0 인 refusal 은 유실 의심 → 콜드 스타트 1회 재시도(D-13). v0.8.3 — §6 hermes 행 (a′) provenance 부재(빈 객체)는 유실(스파이크 4c 실측 2). v0.8.2 — §6 claude_code 유실 판정을 실측대로(`-32002` 또는 `not found`; 스파이크 4c). v0.8.1 — §10 cli_wrapper 치환 범위를 브리프+턴 프롬프트로(D-7 워커 지적). v0.8은 §10 **도구 표면**(`tool_surface`: claude_code `mcp`, hermes `cli_wrapper` — 데몬이 attempt별 래퍼를 만들고 브리프 [2]에 절대 경로) + §9 광고. G5 (b) 차단 결함. v0.7.1은 §11 `estimated`의 자리별 표현(`usage.report` 생략 / `finish` 0 + 무시). v0.7은 §11 `cost_usd` 부재 규칙(생략 + `estimated:true`)과 가격표 소유(워크스페이스=서버). v0.6은 §2.1 시스템 최소에 `USER`(G4 차단 결함: OAuth 갱신 시 키체인이 `USER`를 쓴다). v0.5는 §9 `supported_options`(T-W2가 찾은 빈칸: 프로파일 옵션 능력을 광고할 키가 v0.4.1 대조에서 빠졌다). v0.3 은 PR #20(데몬 P1) 구현·리뷰에서 드러난 5건 반영(usage 시점, Hermes 모델 접두어, Hermes 본문 오류 접두어 규칙, disallowedTools 도출, 250ms 비주입). v0.2는 스파이크 1b 반영 |
...
| 변수 | 값 |
|---|---|
| `COLAB_TASK_TOKEN` | `daemon-protocol.md` §5 — 이 attempt 전용. **번들에 토큰이 없으면(테스트 채팅, daemon-protocol v0.8 §4.5) `COLAB_*` 전부를 넣지 않고 `mcpServers`·래퍼(§10)도 만들지 않는다** — 에이전트는 플랫폼에 말할 수 없고 순수 응답만 한다(v0.8.8) |
| `COLAB_SERVER_URL`(오리진), `COLAB_TASK_ID`, **`COLAB_TASK_ATTEMPT`**, `COLAB_LANE_ID`, `COLAB_SESSION_ID`, `COLAB_ROOM_ID`, `COLAB_WORK_ID`(미션 밖 턴이면 없음, v0.9.1), **`COLAB_THREAD_ID`**(번들 `task.thread_root_id` — 스레드에서 시작된 턴에만, 없으면 키 자체를 넣지 않는다, v0.9.3), `COLAB_AGENT_NAME` | colab CLI/MCP가 쓴다(`colab-cli.md` §1). attempt는 멱등키 파생에 필요 |
...
- `stall` 판정: `running` 상태에서 **`session/update` · 권한 요청 · 원시 스트림 이벤트(claude_code `_claude/sdkMessage` — `message_start`·`content_block_delta` 등) 중 어느 것도 없이 3분** → `failure_kind=stall`, 재시도(FR-7.1). **v0.8.9(S-66, T-D12 실측)**: 이 문장은 v0.8.8 까지 "`session/update`도 권한 요청도 없이" 였는데, claude_code 는 **툴 입력(예: 6.5KB `Write`)을 생성하는 동안 `session/update` 를 한 건도 보내지 않는다**(실측 40.1초 동안 0건, 권한 요청은 입력 생성이 끝난 뒤 온다) — 원시 스트림만 872건(최대 간격 1.3초) 흐른다. 약 160B/s 이므로 30KB 문서 하나가 190초 > 180초가 되어 **긴 글을 쓰는 턴이 전부 stall 로 잘렸다**(Director 실사용 6/6). 뜻은 처음부터 "런타임이 살아서 무언가 하면 stall 이 아니다" 이므로 원시 스트림도 활동으로 센다. 따라서 claude_code 의 원시 스트림(§9 `usage_midturn`, D-17)은 **예산 검사와 stall 활동 두 목적**이라 **항상 켠다** — 예산 없는 번들에서 끄던 D-18 tier 2 는 폐기, 운영자 `usage_midturn:false` 만 끈다. hermes 는 원시 스트림이 없어 같은 상황이 남는다(백로그 D-26). **D-26 의 답(v0.8.11, PR #258 리뷰)**: 임계를 늘리는 것이 아니라 **어댑터가 활동 신호를 흘리는 것**이다 — hermes ACP 어댑터가 긴 툴 입력 생성 중 `session/update` keepalive 를 보내면 기존 리셋 지점이 그대로 동작한다. 조건: keepalive 는 **런타임의 실제 출력에 연동**되어야 하고 타이머만으로 보내면 안 된다(진짜 멈춤에서 stall 이 영영 안 걸린다). 어댑터 upstream 요청 사항이며 데몬은 바꾸지 않는다.
...
**MCP 툴 목록은 역할 부분집합이다(v0.8.10, K-19).** `session/new.mcpServers` 에 싣는 colab MCP 서버는 `--allow <cmd,cmd,…>`(번들 `task.allowed_commands`)로 띄우고, 서버는 그 목록의 툴만 등록한다(`colab-cli.md` §2.5·§3). hermes 래퍼는 같은 목록을 `COLAB_ALLOWED_COMMANDS` 로 export 해 CLI 가 exit 3 로 거부한다. 브리프 [2] 에도 허용 명령만 적는다 — 없는 명령을 지시문이 말하면 에이전트가 시도하고 거부당하는 잡음이 생긴다.

| 런타임 | 브리프 | workdir 파일 |
|---|---|---|
| `claude_code` | `_meta.systemPrompt.append` — **파일을 쓰지 않는다** | 없음. `CLAUDE.md`를 만들거나 고치지 않는다 |
| `hermes` | **`<workdir>/COLAB_BRIEF.md`** — 미추적 별도 파일에 브리프 전문(마커 구간 `<!-- colab:brief:start -->` … `<!-- colab:brief:end -->` 로 감싼다). 턴 프롬프트 **맨 앞 한 줄**이 그 절대 경로를 읽으라고 지시한다(v0.8.6, 스파이크 5 우회 B). **v0.9.7**: `<workdir>` 는 번들 `workdir.path`(에이전트 폴더 = cwd) — 미션 공용 `_shared`(`workdir.shared_path`)에는 **쓰지 않는다**(동료가 읽는 자리에 남의 브리프가 섞인다). `dir` 폴더는 같은 미션·같은 에이전트의 병렬 lane 이 **함께 쓰므로**(daemon-protocol §6.1, D3 A) 파일 이름을 **`COLAB_BRIEF-<lane_id 앞 8자리>.md`** 로 한다 — 한 이름이면 한 lane 이 다른 lane 의 브리프를 덮어쓰고 lane 종료 삭제가 살아 있는 lane 의 파일을 지운다. `worktree` 는 같은 에이전트 lane 이 순차라(E2-12) `COLAB_BRIEF.md` 그대로 | `.git/info/exclude` 에 등록. 추적 중인 `AGENTS.md`·`CLAUDE.md` 는 **읽지도 쓰지도 않는다** — `skip-worktree` 는 쓰지 않는다(에이전트의 정당한 편집을 조용히 삼키고 `switch`·`merge` 를 막는다, SPIKE_05 §3). lane 종료 시 파일 삭제 + exclude 등록 해제. 파일이 이미 있으면(재개·재시도) 덮어쓴다 |
...
브리프 [1]~[8]의 순서는 고정이고 [1]~[5]는 같은 방·같은 미션 안에서 바이트 동일(캐시 친화, E12-11). 턴 프롬프트의 `<trigger>` 는 메시지마다 `<message id author at [thread]>` — `thread` 는 그 메시지가 스레드 답글일 때 루트 id(v0.9.3). 그래서 **턴마다 바뀌는 값은 [1]~[5] 에 넣지 않는다** — 참여자의 현재 상태(working/idle)와 종료 조건 진행(충족 n/m)은 턴 프롬프트(`<roster_status>`·`<mission_progress>`)에 싣는다(v0.9.2). [5] Roster 는 참여자별 이름·역할 설명·멘션 링크만.

**`<folders>` 블록 (v0.9.7, daemon-protocol v0.10.0 §6.1 — Director 승인 2026-09-26 D2 A·D3 A·D4 A·D5 A).** 같은 미션의 동료가 서로의 결과를 폴더로 찾게 하는 턴 프롬프트 블록이다(PRD FR-6.1 — 같은 미션 안은 폴더로, 미션·방 경계를 넘을 때와 리뷰·병합 대상은 아티팩트로). 미션·동료 구성은 턴마다 바뀌므로(참여자 추가, 미션 밖 턴) **브리프가 아니라 턴 프롬프트**다 — `<roster_status>` 와 같은 층.

- **쓰는 쪽은 서버**다. 경로를 서버가 짓고 저장하므로(daemon-protocol §4.1·§6.1) 자리표시자가 필요 없다 — 데몬은 이 블록을 치환하지 않는다(cli_wrapper 의 `colab ` 접두 치환(v0.8.1)은 블록 안에도 평소대로 적용된다).
- **자리**: `<roster_status>` 바로 뒤, `<trigger>` 앞. hermes 의 `COLAB_BRIEF.md` 포인터 줄은 데몬이 프롬프트 **맨 앞**에 붙이는 그대로다(v0.8.6) — 이 블록은 그 뒤 본문 안에 있다.
- **모양**(한 줄 한 폴더, 경로는 절대 경로 — 번들 `workdir.path`·`shared_path` 와 같은 값):

```
<folders>
you: <workdir.path>  (your working folder — write here)
shared: <workdir.shared_path>  (everyone on this mission reads and writes)
- <동료 에이전트 이름>: <그 동료의 폴더>  (read only)
Your other lanes on this mission work in this same folder at the same time: put your lane label `<lane_id[:8]>` in the name of every new file you create.
<마지막 문장 — 표면별>
</folders>
```
...
| 줄 | 언제 싣나 |
|---|---|
| `you:` | 언제나. 재진입 lane 이 옛 배치(`sessions/…`·`worktrees/…`)의 행을 쓰면 그 저장 경로(D6 A). `worktree` 방이면 체크아웃 |
| `shared:` | 미션에 매인 턴(`task.work_id` 있음)만. 미션 밖 턴이면 줄이 없다 |
| `- <이름>:` | 같은 미션에 **workdir 행이 실제로 있는** 다른 에이전트만(없는 경로를 주면 에이전트가 헛돈다). `none` 은 그 미션의 `role=agent` 행. **`worktree` 격리 방은 동료 줄을 싣지 않는다**(v0.9.8, Lead 판정 2026-09-26 — E13-08: 리뷰·병합 판정 근거는 diff 아티팩트이고 남의 미커밋 체크아웃을 가리키지 않는다. worktree 방의 공유는 저장소 밖 `shared:` 로). 이름은 `<roster_status>` 와 같은 표시 이름, 순서는 이름순(같으면 agent id 순). 미션 밖 턴이면 없다 |
| 갈래 표지 줄 | 같은 workdir 행을 가리키는 **이 에이전트의 lane 이 둘 이상**일 때만(D3 A — 같은 미션의 병렬 lane, 예: 시나리오 A Researcher 3-way). `<lane_id[:8]>` 는 이 턴의 lane. 한 폴더에서 같은 파일을 덮어쓰지 않게 하는 규약이다 |
| 마지막 문장 | 언제나 |

- **마지막 문장은 도구 표면별**(v0.9.6 규칙 — 서버가 만드는 모든 텍스트):
  - `cli_wrapper`(hermes): ``Folders of other missions and other rooms are not listed here: ask for an artifact, or use `colab room read`.``
  - `mcp`(claude_code): ``Folders of other missions and other rooms are not listed here: ask for an artifact, or use the `colab_room_read` tool.``
- 토큰 없는 번들(테스트 채팅, v0.8.8)에는 블록이 없다.
- **강제하지 않는다(D5 A).** 형제 폴더 쓰기를 `permissions.deny`(§3)로 막지 않는다 — hermes 에는 같은 수단이 없어 런타임별로 강제가 반쪽이 되고, 반쪽 강제는 규약보다 약속이 커 보인다. 위반은 사후 관찰(mtime·git)로만 보인다.

**브리프 [2] 의 폴더 한 줄 (v0.9.7).** [2] 에 **고정 문장 한 줄**만 더한다 — 경로는 싣지 않는다(경로는 미션마다 달라 [2] 에 넣으면 바이트 동일이 깨진다). 미션 유무·격리와 무관하게 **언제나 같은 바이트**라 같은 에이전트·같은 표면의 [1]~[5] 바이트 동일(E12-11)을 지킨다. 표면별 두 벌:

  - `cli_wrapper`: ``- Your folders are listed in the turn prompt's `<folders>`: your working folder, this mission's shared folder and your mission peers' folders. Write only in your own folder and the shared folder; read your peers' folders but do not edit them. Anything from another mission or another room comes as an artifact or through `colab room read`.``
  - `mcp`: ``- Your folders are listed in the turn prompt's `<folders>`: your working folder, this mission's shared folder and your mission peers' folders. Write only in your own folder and the shared folder; read your peers' folders but do not edit them. Anything from another mission or another room comes as an artifact or through the `colab_room_read` tool.``

  `room_read` 는 모든 역할에 허용이므로(colab-cli §2.5) 역할 부분집합(v0.8.10) 필터가 이 줄을 떨어뜨리지 않는다. 최종 산출물은 여전히 `artifact submit`(v0.9.4 [2] 줄) — 폴더 읽기는 맥락을 얻는 길이고, 리뷰·병합의 판정 근거는 제출된 아티팩트다(PRD FR-4.3).
...
| (v0.9.7, 서버 쪽) 같은 에이전트·같은 표면이면 [2](폴더 줄 포함)가 미션 안 턴·미션 밖 턴에서 같은 바이트 · `<folders>` 가 `<roster_status>` 뒤 `<trigger>` 앞 · 행 없는 동료 줄 0 · `worktree` 방 동료 줄 0(v0.9.8) · mcp 표면 마지막 문장에 셸 `colab ` 0 | E12-11 |
