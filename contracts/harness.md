...
| `- <이름>:` | 같은 미션에 **workdir 행이 실제로 있는** 다른 에이전트만(없는 경로를 주면 에이전트가 헛돈다). `none` 은 그 미션의 `role=agent` 행, `worktree` 방은 그 미션에 lane 이 있는 에이전트의 체크아웃 행. 이름은 `<roster_status>` 와 같은 표시 이름, 순서는 이름순(같으면 agent id 순). 미션 밖 턴이면 없다 |
...
| (v0.9.7, 서버 쪽) 같은 에이전트·같은 표면이면 [2](폴더 줄 포함)가 미션 안 턴·미션 밖 턴에서 같은 바이트 · `<folders>` 가 `<roster_status>` 뒤 `<trigger>` 앞 · 행 없는 동료 줄 0 · mcp 표면 마지막 문장에 셸 `colab ` 0 | E12-11 |
