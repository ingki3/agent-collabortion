...
| `COLAB_SERVER_URL`(오리진), `COLAB_TASK_ID`, **`COLAB_TASK_ATTEMPT`**, `COLAB_LANE_ID`, `COLAB_SESSION_ID`, `COLAB_ROOM_ID`, `COLAB_WORK_ID`(미션 밖 턴이면 없음, v0.9.1), `COLAB_AGENT_NAME` | colab CLI/MCP가 쓴다(`colab-cli.md` §1). attempt는 멱등키 파생에 필요 |
...
브리프 [1]~[8]의 순서는 고정이고 [1]~[5]는 같은 방·같은 미션 안에서 바이트 동일(캐시 친화, E12-11). 그래서 **턴마다 바뀌는 값은 [1]~[5] 에 넣지 않는다** — 참여자의 현재 상태(working/idle)와 종료 조건 진행(충족 n/m)은 턴 프롬프트(`<roster_status>`·`<mission_progress>`)에 싣는다(v0.9.2). [5] Roster 는 참여자별 이름·역할 설명·멘션 링크만.
