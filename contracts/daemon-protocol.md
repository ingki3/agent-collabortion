...
- 메시지 스트리밍: `message.say` 이벤트는 턴 단위로 합치되, 사람이 보는 지연을 위해 `partial: true`인 중간 이벤트를 **같은 seq 없이** 별도 채널(heartbeat의 `preview` 필드)로 보낸다. 영속되지 않는다(PRD §7 "고빈도 이벤트 비영속").
  ...
  **`preview.text` 의 문단 (v0.10.1, T-BUBBLE — Lead 판정 2026-09-26).** 데몬은 턴 동안의 에이전트 텍스트 조각(`agent_message_chunk`)을 누적하되, **두 조각 사이에 도구 호출(`tool_call`)이 끼었으면 빈 줄 하나(`\n\n`)** 를 넣는다 — 누적이 이미 `\n` 으로 끝나면 모자란 만큼만, 턴 첫 조각 앞에는 넣지 않는다. 연속된 조각은 그대로 붙인다. 모양(`{text, message_id}`)은 그대로이고 새 칸은 없다. 서버는 `text` 를 손대지 않고 `message.delta.text` 로 흘리며, 화면(SCREEN §4.6 「작업 중」 말풍선)은 빈 줄로 **진행 메모 문단**을 나눈다. 이 규칙이 없던 옛 데몬은 조각을 구분 없이 이어 붙여 「…통과합니다.BGM v2 가…」 처럼 문장이 붙었다(실사용 게임 제작 방).

  **`preview.text` 상한 (v0.10.2, #356 리뷰 NN1 — Lead 판정 2026-09-27).** 데몬은 `preview.text` 를 **마지막 16,000자**로 자른다 — 자를 때는 가장 가까운 문단 경계(빈 줄) 뒤에서 자르고(없으면 문자 경계) 맨 앞에 `…(앞부분 생략)` 한 줄과 빈 줄을 붙인다. heartbeat 가 매번 전문을 다시 보내므로(30분 수다스러운 턴 실측: 49,328자 · 턴 누적 SSE 8.4MB) 상한 없이는 프레임이 수백 KB 가 된다. **preview 에만** 적용한다 — 영속되는 `message.say`·finish 본문은 자르지 않는다.
...
<workdir_root>/rooms/<room>/
    ...
    _worktrees/<agent>/              ← worktree 격리 체크아웃 (방×에이전트, 브랜치 colab/<방 slug>-<room_id8>/<agent slug>)
...
- **git 브랜치는 ASCII `workdirs.Slug` 그대로** — `colab/<Slug(room.name)>-<room_id[:8]>/<Slug(agent.name)>` — 방 조각에 **id 앞 8자리를 붙인다**(Lead 판정 2026-09-26: 한글 이름 방은 `Slug` 가 `x` 가 되어 같은 저장소에서 방끼리 브랜치가 겹친다). ref 호환(도구·원격 호스팅이 비ASCII ref 를 다르게 다룬다)이 이유다. v0.9.x 서버가 슬러그 재료로 **그 에이전트의 첫 미션 제목**(`COALESCE(work.title, room.name)`)을 쓰던 것은 PRD FR-6.4(방×에이전트)와 어긋난 결함이었다(FINDING-1) — v0.10.0 은 **방 이름**. 이미 만들어진 체크아웃·브랜치는 행의 저장 경로·브랜치를 그대로 쓴다(새 방·새 에이전트부터).
...
- **브랜치 이름 겹침 — 판정(Lead 2026-09-26)**: 한글 이름 방 둘이 같은 저장소·같은 에이전트를 쓰면 id 없는 ASCII `Slug` 로는 `colab/x/<agent>` 로 겹친다(v0.9.x 에도 있던 성질). 그래서 **새 체크아웃**의 방 조각은 `<Slug(room.name)>-<room_id[:8]>` 다. 이미 만들어진 체크아웃은 행에 저장된 경로·브랜치를 그대로 쓴다(D6 A). 그래도 `git worktree add -b` 가 거절하면 구현은 조용히 넘기지 말고 피드에 드러낸다.
